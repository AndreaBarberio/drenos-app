...
function FeaturedProduct({ title, description, image, ctaText, ctaLink, tags = [] }) {
  return (
    <div className="flex flex-col w-full self-center md:flex-row rounded-2xl overflow-hidden text-center md:text-left">
      ...
      <div className="p-6 flex flex-col justify-between text-center md:text-left">
        ...
        <Link to={ctaLink} className="self-center md:self-start bg-black text-white px-4 py-2 rounded hover:bg-gray-800 transition">
          {ctaText}
        </Link>
      </div>
    </div>
